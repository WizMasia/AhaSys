...
export const buildMarkdownReport = (analysisResult: SystemAnalysisResult | null, fallbackModelName: string): string => {
  ...
  let report = `# 🛡️ 광고 법률 자율 무결성 종합 준법 자문보고서\n\n`;
  const gradeInfo = getCsatGradeInfo(analysisResult.score);
  report += `## 1. 종합 심의 판정 요약\n`;
  report += `* **종합 안전 벌점**: **${analysisResult.score}점** / 100점 만점\n`;
  report += `* **최종 준법 성적**: **${gradeInfo.label}** (${gradeInfo.isPassed ? "합격 - 통과 대상" : "기각 - 반려 대상"})\n`;
  report += `* **기저 인프라 엔진**: \`${analysisResult.modelUsed || fallbackModelName || 'Gemini'}\`\n\n`;
  report += `> 💡 **심의 요지**: ${gradeInfo.desc}\n\n`;
  report += `## 2. 심의 검수 대상 원안 메타 정보\n`;
  report += `* **추론 제품 분류군**: \`${analysisResult.parsedMeta.productType}\`\n`;
  report += `* **조사 특별 법령 영역**: \`${analysisResult.parsedMeta.regulatoryDomain}\`\n`;
  report += `* **유통 예정 마케팅 매체**: \`${analysisResult.parsedMeta.channels}\`\n`;
  report += `* **주요 타겟 세그먼트**: \`${analysisResult.parsedMeta.targets}\`\n`;
  if (analysisResult.agentsActivated && analysisResult.agentsActivated.length > 0) {
    report += `* **참여 심의 서브 에이전트**: ${analysisResult.agentsActivated.map((agent) => `\`${agent}\``).join(', ')}\n`;
  }
  report += `\n`;
  report += `## 3. 세부 위법 제재 조항 검출 및 감점 내역 (${analysisResult.violations.length}건)\n`;
  if (analysisResult.violations.length === 0) {
    report += `> ✔ **축하합니다! 위반 검출 사안이 없어 100% 무해성으로 통과 승인되었습니다.**\n\n`;
  } else {
    analysisResult.violations.forEach((violation, index) => {
      report += `### [위반 ${index + 1}] ${violation.clause}\n`;
      report += `* **감점**: **-${violation.deductionPoints}점** | **위험도**: \`${violation.severity}\`\n`;
      report += `* **위법 위험 소견**: ${violation.description}\n`;
      report += `* **문제 발견 원안 구절**: \`"${violation.originalFragment}"\`\n`;
      report += `* **법적 무해 정정 대안안**: **\`"${violation.replacement}"\`**\n\n`;
    });
  }
  if (analysisResult.imageAlternativeProposal) {
    report += `## 4. 이미지 비주얼 멀티모달 Vision 정밀 교정 보고\n`;
    report += `* **Detected Visual Copys (OCR 검출 텍스트)**: ${analysisResult.imageAlternativeProposal.detectedVisualCopys?.join(', ') || '없음'}\n`;
    report += `* **Visual Violations (시각적 리스크 소견)**: ${analysisResult.imageAlternativeProposal.visualViolations?.join(' / ') || '없음'}\n\n`;
    report += `### 🎨 이미지 우회 대안 처방 시안 가이드라인\n`;
    report += `> ${analysisResult.imageAlternativeProposal.alternativeVisualDraft}\n\n`;
  }
